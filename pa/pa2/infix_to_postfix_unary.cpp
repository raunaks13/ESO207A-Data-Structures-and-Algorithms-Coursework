...
int isValid(string str)
{
    ...
    for(int j = 0;j<len;j++)
    {
        ...
        if(j>0)
        {
            if((a[j].tktype == "operator" && a[j].tkval != SUB_VAL) && 
            (a[j-1].tktype == "operator" && a[j-1].tkval != SUB_VAL))// + +
                return 0;
            
            if((a[j].tktype == "operator" && a[j].tkval != SUB_VAL) && 
            (a[j-1].tktype == "parentheses" && a[j-1].tkval == LEFT_VAL))// ( +
                return 0;
    ...
    if(a[len-1].tktype == "operator" || (a[0].tktype == "operator" && a[0].tkval != SUB_VAL))
        return 0;
...
int main()
{
    ...
    for(int k = 1 ; k <= t ; k++)
    {
        ...
        for(int q = 0;q< static_cast <int> (str.length());q++)
        {
            ...
            if(q!= 0)
            {
                if(next == "-" && get_next_token(prev).tktype == "operator")
                    next = "~";
                else if(next == "-" && prev == "(")
                    next = "~";
                prev = next;
